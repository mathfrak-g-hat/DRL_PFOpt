...
############################################
### 2023/05/04, A.J. Zerouali
# An abstract agent class to interact with
# the PortfolioOptEnv class.
'''
TO DO (22/10/27): 
...
'''
                        #######################################################
                        ##### ABSTRACT PORTFOLIO OPTIMIZATION AGENT CLASS #####
                        #######################################################
                        ## 23/05/09
'''
class PFOpt_Agent:
    """
        Abstract portfolio optimization agent class.
        Agent here refers to the reinforcement learning meaning
        of the term, it's the module that does the portfolio
        rebalancing, and/or the prediction of the portfolio
        returns.
        The present agent class implements an abstraction
        that interacts with the PortfolioOptEnv class.
        
        Attributes
        ----------
        :param train_env: PortfolioOptEnv, portfolio environment used for
                        training the model.
                        
        :param test_env: PortfolioOptEnv, portfolio environment used to
                        run a backtest deploying the trained model.
                        
        :attr model: Custom attribute. Stores the objects and hyperparameters
                    that are modified during the execution of the main
                    learning algorithm (train_model()).
        
        :attr model_fname: str, path and filename to save trained agent model.
        
        :attr model_is_trained: bool, indicator for whether or not the method
                        train_model() has been successfully executed. Used
                        as condition for running backtests and saving.
                        
        :attr ran_backtest: bool, indicator for whether or not the method
                        run_backtest() has been successfully executed.
                        Used as condition to store/save backtest results.
                        
        :attr pf_value_hist: pd.DataFrame of portfolio values by date/timestamp
                        during the execution of a backtest. Wraps the
                        PortfolioOptEnv.get_pf_value_hist() in this abstract class.
                        
        :attr pf_return_hist: pd.DataFrame of portfolio returns by date/timestamp
                        during the execution of a backtest. Wraps the
                        PortfolioOptEnv.get_pf_return_hist() in this abstract class.
                        
        :attr pf_weights_hist: pd.DataFrame of portfolio asset weights by date/timestamp
                        during the execution of a backtest. Wraps the
                        PortfolioOptEnv.get_pf_weights_hist() in this abstract class.
                        
        :attr pf_agt_action_hist: pd.DataFrame of PFOpt_Agent actions by date/timestamp
                        during the execution of a backtest. Wraps the
                        PortfolioOptEnv.get_agt_action_hist() in this abstract class.
                        
        :attr pf_performance_stats: pd.DataFrame of portfolio performance stats at the
                        end of execution of run_backtest()
    ...
    def __init__(self, 
        ...
        """
            General constructor 
            :param train_env: PortfolioOptEnv, training environment.
            :param test_env: Optional PortfolioOptEnv. Backtesting environment.
    ...
    def set_test_env(self, test_env: PortfolioOptEnv):
        '''
            Set test environment and reinitialize the portfolio history and performance.
        '''
    ...
    def set_model(self):
        """
            Abstract method. In subclasses, this
            method should be used to fix the hyperparameters
            of the agent model (e.g. DRL algorithm used, actor/critic
            networks architecture.)
            
            NOTE: In subclasses, it could be useful to have the
                  following parameters for this method:
                  set_model(self,
                          model_name,
                          policy="MlpPolicy",
                          policy_kwargs=None, 
    ...
    def train_model(self):
        """
            Abstract method. In subclasses, this method either
            implements or wraps the main learning algorithm.
            
            NOTE: To avoid issues, it is recommended to end the
                  subclass implementation of this method with
    ...
    def save_model(self, model_fname: str = None):
        """
            Abstract method. In subclasses, this method is implemented
            to save the trained model parameters in an appropriate
            format, for instance:
            - For PyTorch models, this method should wrap the
              torch.save(model.state_dict(), PATH) instruction.
              (https://pytorch.org/tutorials/beginner/saving_loading_models.html)
            - For stable_baselines3 models, this method should wrap the
              model.save(path = model_fname) method. This produces a zip file,
              see: https://stable-baselines3.readthedocs.io/en/master/guide/save_format.html
              
              NOTE: To avoid issues, it is recommended to use
                  the model_is_trained attribute to check if the
                  agent has been trained prior to calling this
                  method.
        """
        raise NotImplementedError()
    ...
    def load_model(self):
        """
            Abstract method. In subclasses, this method is implemented
            to load previously saved parameters of a trained model.
            For instance:
            - For PyTorch models, see:
              https://pytorch.org/tutorials/beginner/saving_loading_models.html
            - For stable_baselines3 models, see:
    ...
    def run_backtest(self, 
        ...
        '''
            Abstract method. In subclasses, this method deploys the 
            trained model in the testing environment.
            This method could be implemented in the abstract class,
            but to ensure enough flexibility for the agent model/
            learning environment used, it is better to implement it
            in subclasses. For a concrete example, see the implementation
            of this method in the PFOpt_SB3_Agent class below.
            
            THIS METHOD INITIALIZES MOST OF THE ATTRIBUTES RELATED
            TO BACKTEST PERFORMANCE, MAKE A COMPLETE LIST HERE
...
'''
                            ##########################################
                            ##### STABLE BASELINES 3 AGENT CLASS #####
                            ##########################################
                            ## 23/05/09
'''
...
class PFOpt_SB3_Agent(PFOpt_Agent):
    """
    ...
    def __init__(self, 
        ...
        # Tensorboard log and SB3 callback
